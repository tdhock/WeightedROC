...
roc.curves <- rbind(data.frame(wroc, package="WeightedROC"),
...
for(size in c(400, 800, 1200, 1500, 2000, 2300)){
  ...
  all.sizes <- rbind(all.sizes, data.frame(this.size, size))
}
library(dplyr)
ms <- all.sizes %.% 
  mutate(milliseconds=time/1e6)
algo.stats <- ms %.%
  group_by(expr, size) %.%
  summarise(mean=mean(milliseconds),
            sd=sd(milliseconds))
timePlot <- ggplot()+
  geom_ribbon(aes(size, ymin=mean-sd, ymax=mean+sd, fill=expr), 
              data=algo.stats, alpha=1/2)+
  geom_line(aes(size, mean, color=expr), data=algo.stats, size=2)+
